...
def load_config():
    ...
    config["NTFY_SERVER_URL"] = (
        ...
        或 webhooks.get("ntfy_server_url")
        或 "https://ntfy.sh"
    )
    ...
    # OneBot配置
    config["ONEBOT11_URL"] = os.environ.get("ONEBOT11_URL","").strip() or webhooks.get(
        "onebot1_url",""
    )
    ...
    if config["ONEBOT11_URL"] and config["ONEBOT11_USER_ID"] and config["ONEBOT11_TOKEN"]:
