...
def generate_html_report(
    ...
    # Automatically translate HTML content if GEMINI_API_KEY is available
    import os
    gemini_api_key = os.environ.get("GEMINI_API_KEY")
    if gemini_api_key:
        # Create a temporary txt file from HTML content for translation
        # First convert HTML back to simple text format to translate
        temp_txt_path = file_path.replace('.html', '_temp_for_translation.txt')

        # Extract text content from HTML for translation
        import re
        # This is a simplified extraction - in practice, we'd want more sophisticated HTML parsing
        # But for this project, we can use the existing functionality
        with open(file_path, "r", encoding="utf-8") as f:
            html_text = f.read()

        # For HTML translation, it's better to use the existing translation processor
        # but with HTML as input, so we need to handle it differently
        from translation_processor import TranslationProcessor

        # Define output path for translated HTML
        output_html_path = file_path.replace('.html', '_vi.html')

        # Since we already have the translated text files from save_titles_to_file,
        # we can create a corresponding HTML for the Vietnamese version
        # by processing the corresponding translated txt file
        # For example: if file_path is "output/2023年11月21日/html/10时30分.html"
        # we want to find "output/2023年11月21日/html/10时30分_vi.txt"
        translated_txt_path = file_path.replace('.html', '_vi.txt')

        # If translated txt exists, create corresponding HTML
        if os.path.exists(translated_txt_path):
            # Process the translated txt to generate HTML
        ...
        """
                        ...
                        html_parts.append(platform_html.strip())
...
<head>
    ...
    <title>Translated News Report (Vietnamese)</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1 class="header-title">Vietnamese Translation Report</h1>
        </div>
        <div class="content">
            {''.join(html_parts)}
        </div>
            ...
            print(f"Auto-translated HTML saved to: {output_html_path}")
...
def translate_and_send_email():
    """Translate content and send Vietnamese email if GEMINI_API_KEY is available"""
    import os
    from translation_processor import TranslationProcessor
    ...
    if txt_files:
        ...
        # Create translation processor and run
        processor = TranslationProcessor(api_key=gemini_api_key)
        success = processor.process_all_chunks(latest_txt_file, output_path)
...
def send_translated_email_if_configured(translated_file_path):
    """Send email with translated content if email configuration is available"""
    import os

    # Get email configuration from environment variables (using same names as main.py)
    ...
    # Use the same SMTP configuration logic as main.py
    domain = from_email.split("@")[-1].lower()

    # SMTP configuration (same as main.py)
    SMTP_CONFIGS = {
        # Gmail（使用 STARTTLS）
        "gmail.com": {"server": "smtp.gmail.com", "port": 587, "encryption": "TLS"},
        # QQ邮箱（使用 SSL，更稳定）
    ...
    if smtp_server and smtp_port:
        ...
        # According to port, determine encryption: 465=SSL, 587=TLS
        if smtp_port == 465:
            use_tls = False
        elif smtp_port == 587:
            use_tls = True
        else:
            use_tls = True  # Default to TLS for other ports
    elif domain in SMTP_CONFIGS:
        # Use preset config
        config = SMTP_CONFIGS[domain]
        smtp_server = config["server"]
    ...
    # Create email message (same structure as main.py)
    from email.mime.text import MIMEText
    from email.mime.multipart import MIMEMultipart
    from email.header import Header
    from email.utils import formataddr, formatdate, make_msgid
    ...
    # Set From header as per RFC standard
    sender_name = "TrendRadar Vietnamese Translation"
    msg["From"] = formataddr((sender_name, from_email))
    ...
    subject = f"TrendRadar Vietnamese Translation - {now.strftime('%m月%d日 %H:%M')}"
    msg["Subject"] = Header(subject, "utf-8")
    ...
    text_content = f"""
TrendRadar Vietnamese Translation Report
========================
Generated Time: {now.strftime('%Y-%m-%d %H:%M:%S')}

Please use an HTML-capable email client to view the full report content.
    """
    text_part = MIMEText(text_content, "plain", "utf-8")
    msg.attach(text_part)

    # For the translated file, read content and add as HTML
    with open(translated_file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Convert plain text to simple HTML for email display
    html_content = convert_translated_file_to_html(content)
    html_part = MIMEText(html_content, "html", "utf-8")
    msg.attach(html_part)

    print(f"Sending Vietnamese email to {to_email}...")
    ...
    try:
        ...
        print(f"Vietnamese email sent successfully with content from: {translated_file_path}")
        return True

    except smtplib.SMTPServerDisconnected:
        print(f"Email sending failed: Server disconnected unexpectedly, please check network or try later")
        return False
    except smtplib.SMTPAuthenticationError as e:
        print(f"Email sending failed: Authentication error, please check email and password/app code")
...
def convert_translated_file_to_html(content):
    """Convert translated text file content to simple HTML for email"""
    lines = content.split('\n')
    html_lines = ['<div style="font-family: Arial, sans-serif; line-height: 1.6; max-width: 800px; margin: 0 auto; padding: 20px;">']

    for line in lines:
        line = line.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')

        # Detect if this is a platform title
        if ' | ' in line and not line.startswith(' ') and '.' not in line.split(' | ')[0] and not line.startswith('http'):
            html_lines.append(f'<h2 style="color: #2c5aa0; border-bottom: 2px solid #e0e0e0; padding-bottom: 5px;">{line}</h2>')
        # Detect numbered news items
            ...
            if url_match:
                url = url_match.group(1)
                clean_line = re.sub(r'\[URL:.*?\]', '', line).strip()
                html_lines.append(f'<p><strong>{clean_line}</strong> <a href="{url}" style="color: #1a73e8; text-decoration: none;">🔗</a></p>')
            else:
...
def main():
    try:
        ...
        # After the main crawler finishes, run translation if API key is available
        translate_and_send_email()
