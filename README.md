...
- **每日更新**：自动生成 `rss.xml` 文件，可通过 GitHub Actions 实现每日更新。
- **配置简单**：在 `config.yaml` 中启用并配置 RSS 的 URL。
- **兼容性**：生成的 RSS Feed 可用于任何标准的 RSS 阅读器。

> 💡 **使用方法**：在 `config.yaml` 中将 `enable_rss` 设置为 `true`，并配置好 `base_url` 和 `site_url`。`rss.xml` 文件会生成在项目根目录。
