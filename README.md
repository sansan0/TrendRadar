...
[![Bark通知](https://img.shields.io/badge/Bark-通知-00D4AA?style=flat-square)](https://bark.day.app/)
...
支持**企业微信**(+ 微信推送方案)、**飞书**、**钉钉**、**Telegram**、**邮件**、**ntfy**、**Bark**，消息直达手机和邮箱
...
2. **设置 GitHub Secrets（选择你需要的平台）**:
   ...
   **相关链接：**
   ...
   <details>
   <summary>👉 点击展开：<strong>Bark 推送</strong>（iOS 专用，简洁优雅）</summary>
   <br>
   ...
   **快速开始：**
   ...
   1. **下载 Bark 应用**：
      - iOS：[App Store](https://apps.apple.com/app/bark/id1403753865)

   2. **获取设备密钥**：
      - 打开 Bark 应用，首页会显示一个地址，格式如：`https://api.day.app/xxxxxxxxxxxxxxxxxxxxxx`
      - 其中 `xxxxxxxxxxxxxxxxxxxxxx` 就是你的设备密钥
      - 也可以直接复制完整 URL（程序会自动提取密钥）

   3. **配置 GitHub Secret（⚠️ Name 名称必须严格一致）**：
      - **Name（名称）**：`BARK_DEVICE_KEY`（请复制粘贴此名称，不要手打）
      - **Secret（值）**：填写你的设备密钥或完整 URL

      - **Name（名称）**：`BARK_SERVER_URL`（可选配置，请复制粘贴此名称）
      - **Secret（值）**：留空（默认使用官方服务 `https://api.day.app`）

      - **Name（名称）**：`BARK_GROUP`（可选配置，请复制粘贴此名称）
      - **Secret（值）**：留空（默认使用 `TrendRadar` 作为分组名称）

      - **Name（名称）**：`BARK_SOUND`（可选配置，请复制粘贴此名称）
      - **Secret（值）**：留空（默认使用 `bell`，可选：`alarm`、`alert` 等）

      - **Name（名称）**：`BARK_ICON`（可选配置，请复制粘贴此名称）
      - **Secret（值）**：留空（iOS 15+ 支持自定义图标 URL）

      **说明**：Bark 至少需要配置 1 个必需 Secret (BARK_DEVICE_KEY)，其他为可选配置

   4. **测试**：
      ```bash
      curl -X POST https://api.day.app/你的设备密钥 \
        -H 'Content-Type: application/json' \
        -d '{"title":"测试标题","body":"测试内容"}'
      ```
   ...
   **配置 TrendRadar**：
   ```yaml
   BARK_SERVER_URL: https://bark.yourdomain.com
   BARK_DEVICE_KEY: 你的设备密钥
   BARK_GROUP: TrendRadar  # 可选
   BARK_SOUND: bell  # 可选
   BARK_ICON: https://example.com/icon.png  # 可选，iOS 15+
   ```
   ...
   **相关链接：**
   ...
   - [自托管教程](https://github.com/Finb/bark-server)
...
flowchart TD
    ...
    D --> E[选择通知方式：<br/>📱企业微信 💬飞书 🔔钉钉<br/>📟Telegram 📧邮件 🍎Bark]
